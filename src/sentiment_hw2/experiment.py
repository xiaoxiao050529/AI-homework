...
def build_main_model_specs(embedding_matrix: torch.Tensor) -> List[ModelSpec]:
    ...
    return [
        ...
        build_transformer_spec(embedding_matrix),
    ]


def build_transformer_spec(embedding_matrix: torch.Tensor) -> ModelSpec:
    """定义轻量 Transformer 编码器基线。"""
    return ModelSpec(
        name="transformer",
        builder=lambda: TransformerEncoderClassifier(
            embedding_matrix,
            model_dim=128,
            num_heads=4,
            num_layers=1,
